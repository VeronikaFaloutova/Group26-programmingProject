...
void mousePressed() {
  ...
  if (clicked != null) {
    ...
    if (clicked.label.equalsIgnoreCase("Cancelled")) {
      ...
      dataMap = countByState(currentFilteredFlights);
      piechartScreen = new Screen(color(30,40,60));
      piechartScreen.addWidget(new Widget(1005, 650, 140,35, "Back", colourArray[0]));
      piechartScreen.chart = new PieChart(dataMap);
      currentScreen = piechartScreen;

    } else if (clicked.label.equalsIgnoreCase("Heatmap")) {
      heatmapScreen = new Screen(color(30,40,60));
      dataMap = countByState(currentFilteredFlights);
      heatmapScreen.heatmap = new HeatMap(dataMap);
      heatmapScreen.addWidget(new Widget(1005, 650, 140, 35, "Back", colourArray[0]));
...
LinkedHashMap<String, Integer> countByState(ArrayList<Flight> flights) {
